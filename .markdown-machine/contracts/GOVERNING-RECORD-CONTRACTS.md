...
`TASK_CONTRACT` may bind local context applicability without creating a new authority plane. `project_context_refs`, `subtree_context_bindings[{context_ref,path}]`, and `exact_path_context_bindings[{context_ref,path}]` exact-bind project-local `CONTEXT_REPRESENTATION` records and `canonical_logical_path` values as part of the already-admitted current Task. Free-form `scope` remains descriptive and cannot substitute for these exact mappings. Absence of an optional mapping means an empty local mapping, not permission for model inference.
