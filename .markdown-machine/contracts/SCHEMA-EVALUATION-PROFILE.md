...
Complete project-exportable primitive/schema algebra. String constructor descriptors (`enum(...)`, `list<T>`, and `object<fields>`) and structured typed-reference descriptor keys are closed here and must exactly agree with `MM-CONTRACT-EVAL/1`.
