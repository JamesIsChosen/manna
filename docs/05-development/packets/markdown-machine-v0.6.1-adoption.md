...
This packet records a direct governed v0.6.1 adoption successor constructed
from the legitimate pre-adoption Manna design tip. Product source and product
decisions are unchanged. The failed v0.5.1 migration and failed v0.6.0
adoption remain immutable historical evidence.
...
from this tip, so its adoption diff is isolated from the pending design PR.
...
not a fabricated second confirmation.

## Adoption decision

The prior Manna subject has no provable usable current-law migration lineage.
The old-law probe therefore remains `NO_PROVABLE_LINEAGE`, with historical
status `HISTORICAL_UNVERIFIED`. The lawful operation is direct `ADOPTION`, not
`KERNEL_MIGRATE`, with one new epoch-0/sequence-0 positive v0.6.1 Genesis.
No legacy bridge or historical convergence capacity is reused.
